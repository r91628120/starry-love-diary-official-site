...
export function HomePage() {
  ...
  return <>
    <section className="hero section" id="top"><div className="hero__copy"><p className="eyebrow">{siteContent.brand.en}</p><h1>{hero.title}</h1><h2>{hero.subtitle}</h2><p>{hero.body}</p><div className="hero__actions"><a className="button button--primary" href="#philosophy">{hero.primary}</a><a className="button button--secondary" href="#features">{hero.secondary}</a></div><span className="coming-soon">✦ App {hero.comingSoon}</span></div><HeroArtwork /></section>
    <section className="section philosophy" id="philosophy"><div className="philosophy__scenery" aria-hidden="true"><span className="philosophy__moon">☾</span><span className="philosophy__skyline" /><span className="philosophy__pair">● ●</span><span className="philosophy__cat">⌁</span></div><div className="philosophy__copy"><p className="eyebrow">給喜歡著某個人的你</p><h2>{philosophy.title}</h2><p>{philosophy.body}</p><p>{philosophy.answer}</p><ul className="philosophy__highlights">{philosophy.highlights.map((item) => <li key={item}>{item}</li>)}</ul></div></section>
